...
class Surrogate:
    ...
    def assess_sm(self, input_list, output_list):
        ...
        for i in range(0, len(input_list)):
            ...
            # Gets the expected curve
            exp_x_end = output_list[i][0]
            exp_polynomial = output_list[i][1:]
            exp_x_list = list(np.linspace(0, exp_x_end, num = DATA_DENSITY, endpoint = True))
            exp_y_list = list(np.polyval(exp_polynomial, exp_x_list))

            # Gets the predicted curve
            prd_x_end = prd_output_list[i][0]
            prd_polynomial = prd_output_list[i][1:]
            prd_x_list = list(np.linspace(0, prd_x_end, num = DATA_DENSITY, endpoint = True))
            prd_y_list = list(np.polyval(prd_polynomial, prd_x_list))

            # Gets the end point errors
            ...
            min_x_list = list(np.linspace(0, min_x_end, num = DATA_DENSITY, endpoint = True))
            exp_y_list = list(np.polyval(exp_polynomial, min_x_list))
            prd_y_list = list(np.polyval(prd_polynomial, min_x_list))
            err_area = np.average([abs(exp_y_list[j] - prd_y_list[j]) / exp_y_list[j] for j in range(0, DATA_DENSITY)])
...
class Sampler:
    ...
    def __init__(self):
        ...
        # Gets the list of end-points and polynomial coefficients (outputs)
        columns = ['x_end'] + ['c_' + str(term) for term in range(0, polyfier.POLY_DEG+1)]
        self.output_list = [xl.read_column(column = column) for column in columns]
        self.output_list = [[output[i] for output in self.output_list] for i in range(0,len(self.output_list[0]))] # transposes
